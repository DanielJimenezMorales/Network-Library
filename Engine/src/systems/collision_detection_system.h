...
namespace Engine
{
	...
	class CollisionDetectionSystem : public ECS::ISimpleSystem
	{
		...
		private:
			...
			/// <summary>
			/// <para>Sort all potential colliders using X coordinate from left to right in order to be able to apply
			/// future optimizations.</para>
			/// </summary>
			...
			/// href="https://dyn4j.org/2010/01/sat/">this link</see></para> <para>NOTE: In case of collision, this
			/// method resturns true and mtv contains the magnitude and normalized direction of the same minimum
			...
			/// </summary>
			/// <returns></returns>
			float32 GetProjectionsOverlapMagnitude( float32 minProjection1, float32 maxProjection1,
			...
			/// <summary>
			/// <para>Applies a collision response based on the mtv in order to separate both colliding shapes. This
