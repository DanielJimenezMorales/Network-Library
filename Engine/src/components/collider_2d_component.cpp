...
namespace Engine
{
	...
	void Collider2DComponent::GetAxes( ReadOnlyTransformComponentProxy& transform, std::vector< Vec2f >& outAxes ) const
	{
		_bounds->GetAxes( transform, outAxes );
	}

	void Collider2DComponent::ProjectAxis( ReadOnlyTransformComponentProxy& transform, const Vec2f& axis, float& outMin,
	                                       float& outMax ) const
	{
		_bounds->ProjectAxis( transform, axis, outMin, outMax );
	}

	Vec2f Collider2DComponent::GetClosestVertex( ReadOnlyTransformComponentProxy& transform,
	                                             const Vec2f& inputPoint ) const
	{
		return _bounds->GetClosestVertex( transform, inputPoint );
	}

	float32 Collider2DComponent::GetMinX( ReadOnlyTransformComponentProxy& transform ) const
	{
		return _bounds->GetMinX( transform );
	}

	float32 Collider2DComponent::GetMaxX( ReadOnlyTransformComponentProxy& transform ) const
	{
