...
namespace Engine
{
	namespace ECS
	{
		...
		{
			return const_cast< GameEntity& >(
			    static_cast< const EntityContainer& >( *this ).GetFirstEntityOfType< T >() );
		}

		// TODO For some reason this method throws a compilation error of type: C2440 '<function-style-cast>': cannot
		// convert from 'initializer list' to 'GameEntity. Edit: This issue has been solved by adding const_cast.
		// However, not a really good quality solution. Dev should think in something better.
		template < typename T >
