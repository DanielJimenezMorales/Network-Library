...
void SpriteRendererSystem::Render( ECS::EntityContainer& entityContainer, SDL_Renderer* renderer ) const
{
	...
	SDL_Rect destRect;
	std::vector< GameEntity > entitiesToRender =
	    entityContainer.GetEntitiesOfBothTypes< SpriteRendererComponent, TransformComponent >();
	...
	for ( ; cit != entitiesToRender.cend(); ++cit )
	{
		...
		SDL_RenderCopyEx( renderer, texture->GetRaw(), &texture->GetDimensions(), &destRect,
