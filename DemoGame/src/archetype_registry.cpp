...
namespace ECS
{
	...
	bool ArchetypeRegistry::RegisterArchetype( const Archetype& archetype )
	{
	...
	bool ArchetypeRegistry::UnregisterArchetype( const std::string& name )
	{
		...
		{
			LOG_ERROR(
			    "[ArchetypeRegistry::UnregisterArchetype] Can't unregister archetype with name %s because it wasn't "
			    "registered",
			    name.c_str() );
		}
	...
	bool ArchetypeRegistry::TryGetArchetype( const std::string& name, Archetype& out_archetype ) const
	{
		bool result = false;
	...
	bool ArchetypeRegistry::IsArchetypeValid( const Archetype& archetype ) const
	{
