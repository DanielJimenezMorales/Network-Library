...
namespace NetLib
{
	...
	public:
		RemotePeer();
		RemotePeer(const sockaddr_in& addressInfo, uint16 id, float32 maxInactivityTime, uint64 clientSalt, uint64 serverSalt);
		...
		void Connect(const sockaddr_in& addressInfo, uint16 id, float32 maxInactivityTime, uint64 clientSalt, uint64 serverSalt);
		...
		uint64 GetDataPrefix() const { return _clientSalt ^ _serverSalt; } //XOR operation to create the data prefix for all packects from now on
