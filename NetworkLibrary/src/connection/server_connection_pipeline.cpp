...
namespace NetLib
{
	namespace Connection
	{
		                                                                 ...
		                                                                 ConnectionFailedReasonType reason )
		{
			...
			if ( message == nullptr )
			{
				LOG_ERROR( "%s Can't create new connection denied Message because the MessageFactory has returned a "
				           "null message",
				           THIS_FUNCTION_NAME );
				return nullptr;
			}
		                                                                   ...
		                                                                   uint64 data_prefix, uint16 id )
		{
			...
			{
				LOG_ERROR(
				    "%s Can't create new Connection Accepted Message because the MessageFactory has returned a null "
				    "message",
				    THIS_FUNCTION_NAME );
				return nullptr;
			}
			...
			if ( pending_connection.GetDataPrefix() == message.prefix )
			{
				if ( pending_connection.GetCurrentState() == PendingConnectionState::Failed )
				{
					outcomeMessage = CreateConnectionDeniedMessage( message_factory,
					                                                pending_connection.GetConnectionDeniedReason() );
				}
				else
				{
					...
					outcomeMessage = CreateConnectionAcceptedMessage(
					    message_factory, pending_connection.GetDataPrefix(), pending_connection.GetId() );
				}
			}
			else
			{
